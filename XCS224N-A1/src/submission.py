...
def distinct_words(corpus):
    ...
    for c in corpus:
        for _c in c:
            if not _c in corpus_words:
                corpus_words.append(_c)
    #print(corpus_words)
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    for corp in corpus:
        length = len(corp)
        j = 0
        for j in range(0, len(corp) - 1):
            for k in range(j + 1, j + 1 + window_size):
                if k < length:
                    first_word = corp[j]
                    second_word = corp[k]
                    index_first_word = word2Ind[first_word]
                    index_second_word = word2Ind[second_word]
                    M[index_first_word, index_second_word] += 1
                    M[index_second_word, index_first_word] += 1
    # ### END CODE HERE ###
