...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  #dscores = prob.copy()
  #dscores[outside_word_idx] = dscores[outside_word_idx] - 1  # y_hat minus y which is 1 in this case
  #grad_center_vec = np.matmul(outside_vectors[outside_word_idx].T, dscores)
  #grad_outside_vecs =  np.outer(dscores, center_word_vec)

  outside_vec = outside_vectors[outside_word_idx]
  grad_center_vec = -outside_vec + np.sum(outside_vectors * prob, axis=0)

  grad_outside_vecs = np.dot(prob, center_word_vec[:, np.newaxis].T)
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  curr_center_word_idx = word2ind[current_center_word]
  center_word_vec = center_word_vectors[curr_center_word_idx]
  ...
  for word in outside_words:
    ...
    grad_center_vecs = grad_center_vecs + gradc
    grad_outside_vectors =  grad_outside_vectors + grado

  grad_center_vecs[np.arange(grad_center_vecs.shape[0]) != grad_center_vecs] = 0
...
def sgd(f, x0, step, iterations, postprocessing=None, use_saved=False,PRINT_EVERY=10):
  ...
  for iter in range(start_iter + 1, iterations + 1):
    ...
    loss, grad = f(x)
    x -= step * grad
      #print('x,loss, prev_loss',x,loss,prev_loss)
